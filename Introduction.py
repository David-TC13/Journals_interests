...
st.write('## Methods')
st.write("""To analyse the influence that a government or an entity can provoke a partial news, we decided to take 4 different broadcast from different locations which can be meaningful in order to establish this relation. 
""")
st.write("""In this particular case, we decided to use 2 sources from the west bloc (BBC,CNN) and two from the east bloc (RT, People's daily) with the idea to check also which one of both sides is more impartial reporting news. 
...
addition two new columns were created: one was to introduce the source of the article and another one for the topic searched and resulted as the article mentioned in that row. The last step of this second stage was creating different
 DF's and save them into CSV's getting all the top 20 words of each broadcast agencies for, once more, being able to do some visualisation about it.
The third step was the visualisation; on one side it was created a wordcloud with each agency's logo and a last one with a reporter's shape. On the other side, it was created a tableau story with all the graphs that were considered 
relevant for this analysis (see page Tableau). """)
st.write("""Finally, the last step completed was implementing this data app; it was separed in 3 pages, one for the explanation, a second one for visualisation on tableau (embedding the story on the app) and a third one for filter. 
The purpose of this last page was to be able to check the news published in a certain day by title, source, topic, the subjectivity and polarity, as well as, including the top 20 most recurrent meaningful words in that article.""")
...
st.write("""As a result of this analysis, it can be noted an average of subjectivity by weeks oscillates around 0.35 (on a scale 0-1) which slightly higher in RT, which in week 5 of 2023, reach 0.4, being the second BBC in week 7 of 2023, with an average of 0.38, although, 
in general the subjectivity found has no significant difference in between them.""")

st.write('#### Polarity')
st.write("""In this case, it can be seen bigger discrepancies between the news agencies; on a scale from -1 to +1, none of them goes above/below 0.1, which means there's not much negativity/positivity inferred in the articles. Regardless, in this range of -0.1 to 0.1, we can 
underline the bigger discrepancies where, for example, RT, is the most negative one (actually, it's the one journal which falls below 0) and CNN the most positive (0.086).
""")
st.write('#### Number of publications by newspaper')
st.write("""To analyse this parameter it needs to take in consideration the topics used; as the intention of this project is to check how geopolitics can cause an impact on their national newspapers, the amount of articles published are related with it; so, for example, BBC and 
...
st.write("""In this case, the analysis is focused on the topic resulting most recurrent, in other terms, what it's talked about more often; in this case, again, it has to be considered the fact of splitting the articles in two blocs: Russia/China vs. UK/USA. Considered this, it's observed that the highest amount of articles published by the western 
newspapers are about Russia, followed by USA.
 """)
st.write('#### Most used words, difference in between the "two blocs"')
st.write("""- East Bloc: Checking the graphs in tableau, it's clear that the most used word is China followed by United and States that, as a compound name, would be the second recurrent word in the eastern newspapers. It was also noted that the use of the term Ukraine, Russia and military are relevant words vs. war appears far lower times (39 times vs. military 79). """)
st.write("""- West Bloc: In this case China and Russia are the most frequent words used, as expected, followed closely by Ukraine. Also noted the term war is used more often (196 times). It appears also relevant, the amount of times  the word Putin emerged(100 times vs 19 on the Easter bloc).
         """)
st.write("""- Overall of words: It's noted a massive significance of China as most recurrent in both blocs, followed by Ukraine, Russia, United and war. Also the relevance of this last word (war) is used more often than military overall.""")
st.write('## Conclusions:')
st.write(""" The first and more relevant conclusion comes from the subjectivity analysis; as it was shown there's overall a significant subjectivity in all the newspapers, without having any outlier. This brings to question if the justitication by the European Union to ban RT to broadcast in this territory.
